...
window.loadPacking=async function(){
  ...
  if(!sh){
    ...
    const ins=await sb.from('sim_pack_shifts').insert({shift_date:qd,created_by:(me&&me.id)||null}).select().single(); if(ins.error){$('packBody').innerHTML='<div class="card"><p class="muted">'+esc(ins.error.message)+'</p></div>';return} sh=ins.data
  }
  packShift=sh
...
function packActionPanel(packing,next){
  if(packing){
    ...
    return `<div class="card" style="background:var(--panel2);border-color:var(--accent);text-align:center;margin:6px 0 0">
      <div style="font-size:12px;color:var(--muted);letter-spacing:.5px">NOW PACKING · SKU ${esc(packing.sku||'–')}${packCompCount(packing.sku)!=null?' · 🧩 '+packCompCount(packing.sku)+' components':''}${packing.line_count?' · '+packing.line_count+' on line':''}</div>
  ...
  if(next){
    ...
    return `<div class="card" style="background:var(--panel2);text-align:center;margin:6px 0 0">
      <div style="font-size:12px;color:var(--muted);letter-spacing:.5px">NEXT UP · SKU ${esc(next.sku||'–')}</div>
      <div style="font-size:19px;font-weight:800;margin:2px 0">${esc(next.dish_name)}</div>
      <div class="muted" style="margin-bottom:2px"><b style="font-size:22px;color:var(--txt)">${next.planned_qty??'–'}</b> to pack · 🧩 <b style="color:var(--txt)">${packCompCount(next.sku)!=null?packCompCount(next.sku):'–'}</b> components <a class="link" style="font-size:13px" data-sku="${esc(next.sku)}" onclick="packSetComponents(this.dataset.sku)">edit</a></div>
...
function packRunRow(r){
  ...
  const skuBlock=`<div style="flex:0 0 auto;text-align:center;min-width:38px"><div style="font-size:10px;color:var(--muted)">SKU</div><div style="font-size:20px;font-weight:900;color:var(--accent);line-height:1">${esc(r.sku||'–')}</div></div>`
  ...
  const compChip=` · <a class="link" style="font-size:13px" data-sku="${esc(r.sku)}" onclick="packSetComponents(this.dataset.sku)">🧩 ${_comp!=null?_comp+' comp':'set comp'}</a>`
...
function packedRunRow(r,actualPos,plannedPos){
  ...
  return `<div class="task-item" data-runid="${r.id}" style="flex-direction:column;align-items:stretch;gap:4px">
    <div style="display:flex;align-items:center;gap:10px">
      ...
      <div style="flex:0 0 auto;text-align:center;min-width:34px"><div style="font-size:10px;color:var(--muted)">SKU</div><div style="font-size:16px;font-weight:900;color:var(--accent);line-height:1">${esc(r.sku||'–')}</div></div>
...
/* The rest of the packing screen lives in two more files, loaded straight
   after this one in index.html. They are separate files only because this one
   grew too large to work on comfortably — they share the same global scope and
   the same pack* state declared at the top of this file:

     packing-reports.js  end-of-day PDF report, history view, plan order PDF
     packing-guides.js   packing guide cards, dish-list staging, line prep sheet

   If a "packEodReport is not defined" style error ever appears, the script tag
   for one of those two files is missing from index.html. */
