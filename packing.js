...
/* Components this dish is actually made of, per the same BOM (sim_dish_bom
   -> sim_components) already used to gate kitchen-readiness. Empty when the
   dish has no BOM recorded yet — packAskTemp falls back to free text then. */
function packComponentOptions(sku){
  const ids=(sku!=null&&packBom[sku])||[]
  return ids.map(id=>packCompName[id]).filter(Boolean)
}

/* CCP temperature gate — required before a dish can be started or stopped.
   Also captures WHICH component was actually probed (picked from the dish's
   BOM when one is known, free text otherwise) — a bare temperature against
   a whole multi-component dish isn't a meaningful record on its own.
   Returns {temp,component,action} (action null when in range), or null if
   cancelled. */
function packAskTemp(stage,dishName,sku){
  return new Promise(resolve=>{
    const verb=stage==='START'?'start':'finish'
    const comps=packComponentOptions(sku)
    const m=document.createElement('div')
    m.style.cssText='position:fixed;inset:0;background:rgba(0,0,0,.65);z-index:1000;overflow-y:auto;padding:20px 12px;display:flex;align-items:center'
    const compField=comps.length
      ? '<select id="ccpComp" style="width:100%;margin-top:4px"><option value="">— select component —</option>'
        +comps.map(c=>'<option value="'+esc(c)+'">'+esc(c)+'</option>').join('')
        +'<option value="__other">Other / not listed</option></select>'
        +'<input id="ccpCompOther" placeholder="Which component?" style="width:100%;margin-top:6px;display:none" />'
      : '<input id="ccpComp" placeholder="e.g. chicken, sauce, rice" style="width:100%;margin-top:4px" />'
    m.innerHTML='<div class="card" style="max-width:380px;margin:0 auto">'
      +'<h2 style="margin:0 0 4px">\u{1F321} CCP CHECK — '+stage+'</h2>'
      +'<p class="muted" style="margin:0 0 10px">"'+esc(dishName)+'" — a reading is required before you can '+verb+' this dish.</p>'
      +'<label class="muted" style="font-size:12px">Component probed'+(comps.length?'':' (this dish has no components set up yet — type what you probed)')+'</label>'
      +compField
      +'<label class="muted" style="font-size:12px;display:block;margin-top:10px">Temperature (°C)</label>'
      +'<input id="ccpTemp" type="text" inputmode="decimal" placeholder="e.g. 3.5" style="width:100%;margin-top:4px" />'
      +'<div id="ccpErr" style="color:#fca5a5;font-size:13px;margin-top:8px"></div>'
      +'<div style="display:flex;gap:8px;margin-top:14px">'
      +'<button class="ghost sm" id="ccpCancel" style="flex:1">Cancel</button>'
      +'<button class="green sm" id="ccpOk" style="flex:1">Record reading</button>'
      +'</div></div>'
    document.body.appendChild(m)
    if(comps.length){
      const sel=m.querySelector('#ccpComp'), other=m.querySelector('#ccpCompOther')
      sel.onchange=()=>{ other.style.display=sel.value==='__other'?'block':'none' }
    }
    function cleanup(v){ m.remove(); resolve(v) }
    m.onclick=(e)=>{ if(e.target===m)cleanup(null) }
    m.querySelector('#ccpCancel').onclick=()=>cleanup(null)
    m.querySelector('#ccpOk').onclick=()=>{
      const err=m.querySelector('#ccpErr'); err.textContent=''
      let comp
      if(comps.length){
        const sel=m.querySelector('#ccpComp').value
        comp=sel==='__other'?m.querySelector('#ccpCompOther').value.trim():sel
      } else {
        comp=m.querySelector('#ccpComp').value.trim()
      }
      if(!comp){ err.textContent='Select or enter which component you probed.'; return }
      const t=m.querySelector('#ccpTemp').value.trim().replace(',','.')
      const n=Number(t)
      if(t===''||isNaN(n)){ err.textContent='Enter the temperature as a number — e.g. 3.5'; return }
      if(n<-30||n>120){ err.textContent='That reads '+n+'°C — outside a believable range. Re-probe and enter it again.'; return }
      if(n>PACK_CCP_LIMIT){
        const act=prompt('⚠️ OUT OF RANGE — '+n+'°C on "'+comp+'" is above the '+PACK_CCP_LIMIT+'°C chilled limit.\n\nA CORRECTIVE ACTION is required to continue — e.g. "returned to chiller, re-probe in 20 min", "moved to blast chiller", "supervisor informed".')
        if(act===null)return
        if(!String(act).trim()){ err.textContent='A corrective action is required when the temperature is above '+PACK_CCP_LIMIT+'°C.'; return }
        cleanup({temp:n,component:comp,action:String(act).trim()})
        return
      }
      cleanup({temp:n,component:comp,action:null})
    }
  })
}
...
function packTempLine(r){
  ...
  if(!s&&!f)return ''
  const sc=r.start_temp_component?' ('+esc(r.start_temp_component)+')':''
  const fc=r.finish_temp_component?' ('+esc(r.finish_temp_component)+')':''
  let h='<div class="muted" style="font-size:12px;margin-top:2px">🌡 CCP: start '+(s?s+sc:'—')+' → finish '+(f?f+fc:(r.status==='packing'?'pending':'—'))+'</div>'
...
function packActionPanel(packing,next){
  if(packing){
    ...
    return `<div class="card" style="background:var(--panel2);border-color:var(--accent);text-align:center;margin:6px 0 0">
      ...
      ${packing.start_temp_c!=null?`<div class="muted" style="font-size:12px;margin-top:2px">🌡 CCP start ${packTempChip(packing.start_temp_c)}${packing.start_temp_component?' ('+esc(packing.start_temp_component)+')':''} · temp check required again at STOP</div>`:''}
...
window.packStartDish=async function(id){
  ...
  const ccp=await packAskTemp('START',r.dish_name,r.sku)
  if(!ccp)return
  ...
  const upd={start_time:new Date().toISOString(),status:'packing',changeover_mins:co,line_count:n,pack_seq:maxSeq+1,start_temp_c:ccp.temp,start_temp_component:ccp.component,start_temp_action:ccp.action}
...
window.packStopDish=async function(id){
  ...
  const ccp=await packAskTemp('STOP',r.dish_name,r.sku)
  if(!ccp)return
  const {error}=await sb.from('sim_pack_runs').update({finish_time:fin,total_minutes:mins,qty_packed:qty,status:'done',finish_temp_c:ccp.temp,finish_temp_component:ccp.component,finish_temp_action:ccp.action}).eq('id',id)
  if(error){alert(error.message);return}
  if(rate!=null){
